package exam.demo.config;

import org.springframework.security.core.Authentication;
import org.springframework.security.core.authority.AuthorityUtils;
import org.springframework.security.web.authentication.AuthenticationSuccessHandler;
import org.springframework.stereotype.Component;

import javax.servlet.ServletException;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.io.IOException;
import java.util.Set;

@Component
public class AuthenticationSuccess implements AuthenticationSuccessHandler {

    @Override
    public void onAuthenticationSuccess(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Authentication authentication) throws IOException, ServletException {

        Set<String> roles = AuthorityUtils.authorityListToSet(authentication.getAuthorities());

        if (roles.contains("ROLE_ADMIN")) {
            httpServletResponse.sendRedirect("/admin");
        }else if (roles.contains("ROLE_RAHBARIYAT")) {
            httpServletResponse.sendRedirect("/rahbariyat");
        } else if (roles.contains("ROLE_SPIKER")) {
            httpServletResponse.sendRedirect("/spiker");
        }
        else if (roles.contains("ROLE_MUROJAAT")) {
            httpServletResponse.sendRedirect("/murojaat");
        }  else if (roles.contains("ROLE_KANSELYARIYA")) {
            httpServletResponse.sendRedirect("/kanselyariya");
        } else {
            httpServletResponse.sendRedirect("/index");
        }
    }
}
